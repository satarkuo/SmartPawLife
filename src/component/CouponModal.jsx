...
function CouponModal ({
    ...
    const updateCoupon = async () => {
        ...
        try {
            const res = await axios.put(`${BASE_URL}/api/${API_PATH}/admin/coupon/${tempModalData.id}`, { data } )
            closeModal();
            dispatch(pushMessage({
                title: '優惠券資料已更新',
                text: res.data.message,
            ...
            <div className="modal-dialog modal-md">
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <label htmlFor="title" className="form-label">主題</label>
                        <div className="input-group mb-3">
                            <input type="text" name="title" id="title" 
                                value={tempModalData.title}
                                onChange={handleModalInputChange}
                                className="form-control" placeholder="請輸入主題"/>
                        </div>
                            ...
                            <div className="col">
                                ...
                                <div className="input-group mb-3">
                                    <input type="text" name="code" id="code" 
                                        ...
                                        onChange={handleModalInputChange}
                                        className="form-control" placeholder="請輸入折扣碼"/>
                                </div>
                            </div>
                            <div className="col">
                                ...
                                <div className="input-group mb-3 d-flex align-items-center">
                                    <input type="number" name="percent" id="percent" 
                                        ...
                                        className="form-control me-2" placeholder="請輸入折扣"/> ％
                                </div>
                            </div>
                        </div>
                        ...
                        <label htmlFor="due_date" className="form-label">
                            到期日
                            {modalMode === 'create' ? <span className='textBody3 ms-2 text-body-tertiary'>預設一個月以後</span> : ''}
                        </label>
                        <div className="input-group mb-3">
                            <input type="date" name="due_date" id="due_date" 
                                value={ date? new Date(date * 1000).toISOString().split("T")[0] : ''}
                                onChange={handleDateChange}
                                className="form-control" placeholder="請選擇到期日"/>
                        </div>
